</div>
          <!-- content-wrapper ends -->
          <!-- partial:partials/_footer.html -->
          <footer class="footer">
  <div class="footer-inner-wraper">
    <div class="d-sm-flex justify-content-center justify-content-sm-between">
      <span class="text-muted text-center text-sm-left d-block d-sm-inline-block">Copyright © 2024 . All rights reserved.</span>
      <span class="float-none float-sm-right d-block mt-1 mt-sm-0 text-center">Visit with wings <i class="mdi mdi-heart text-danger"></i></span>
    </div>
  </div>
</footer>
          <!-- partial -->
        </div>
        <!-- main-panel ends -->
      </div>
      <!-- page-body-wrapper ends -->
    </div>
    <!-- container-scroller -->
    <!-- plugins:js -->
    <script src="../Assets/Templates/Admin/assets/vendors/js/vendor.bundle.base.js"></script>
    <!-- endinject -->
    <!-- Plugin js for this page -->
    <script src="../Assets/Templates/Admin/assets/vendors/chart.js/chart.umd.js"></script>
    <script src="../Assets/Templates/Admin/assets/vendors/jquery-circle-progress/js/circle-progress.min.js"></script>
    <script src="../Assets/Templates/Admin/assets/js/jquery.cookie.js"></script>
    <!-- End plugin js for this page -->
    <!-- inject:js -->
    <script src="../Assets/Templates/Admin/assets/js/off-canvas.js"></script>
    <script src="../Assets/Templates/Admin/assets/js/hoverable-collapse.js"></script>
    <script src="../Assets/Templates/Admin/assets/js/misc.js"></script>
    <script src="../Assets/Templates/Admin/assets/js/settings.js"></script>
    <script src="../Assets/Templates/Admin/assets/js/todolist.js"></script>
    <!-- endinject -->
    <!-- Custom js for this page -->
    <script src="../Assets/Templates/Admin/assets/js/dashboard.js"></script>
    <!-- End custom js for this page -->
  </body>
</html>